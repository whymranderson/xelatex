...
def gfunc(xx):
    ''''''
    gout = np.pi*xx*xx*hydrogenN3l2(xx)*hydrogenN3l2(xx)
    return gout
def gyroshape():
    y = 200*gfunc(xx)
...
Z_vec_array = grr[0]*np.transpose(np.load('outfile.npy'))
...
gx = np.outer(200*gfunc(xx), np.sin(v))
print np.shape(gx)
gz = np.outer(200*gfunc(xx), np.cos(v))
...
gxlr = np.reshape(glrotatedvec[0,:],(50,20))
gylr = np.reshape(glrotatedvec[1,:],(50,20))
gzlr = np.reshape(glrotatedvec[2,:],(50,20))
ax3.plot_surface(gxlr, gylr, gzlr, rstride=2, cstride=2,
                       color='w', linewidth=0,)
                       #alpha=0.1)
# wireframe
...
ax3.plot(*gyrovec,color='orange') #s=markersizearray,

ax3.plot(*Z_vec_array,color='w')#, s=markersizearray*0.05, color='b')
...
plt.show()
#plt.savefig(r'C:\Documents and Settings\user\My Documents\tony\2014\Xelatexfolder\gyrologo5.pgf')
