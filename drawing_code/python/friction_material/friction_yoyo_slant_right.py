...
########
fig3 = plt.figure(3,figsize=(3, 1.5),dpi=100)
...
arrow2 = pat.FancyArrow(
    cm[0]+0.6*R/1.414, cm[1]-0.6*R/1.414,pb[0]*2/3/1.414,pb[0]*2/3/1.414,#increment
    width=0.018,        # Default
    head_width=0.08,    # Default: 3 * width
    head_length=0.09,    # Default: 1.5 * head_width
    color = 'r'
...
ax.add_patch(arrow2)
plt.text(cm[0]+0.6*R/1.414+ pb[0]*2/3/1.414,cm[1]-0.6*R/1.414+pb[0]*2/3/1.414,r'$F_{p}$', fontsize=14,verticalalignment='top', horizontalalignment='left')
plt.text(cm[0]+0.01,cm[1]-0.6*R,r'$\theta$', fontsize=13,verticalalignment='bottom', horizontalalignment='left')

lineVert, = plt.plot([0,0],[0,cm[1]],':')
lineSlant, = plt.plot([cm[0]+0.6*R/1.414,cm[0]],[cm[1]-0.6*R/1.414,cm[1]],'k')
...
ax.add_patch(arrow1)
plt.text(pd[0]-0.15,pd[1]-0.05, r'$1. F_{fr}$', fontsize=14,verticalalignment='top', horizontalalignment='left')

vecfr2= (pc+pb)/2 - pc
arrow3 = pat.FancyArrow(
    pc[0], pc[1], vecfr2[0],vecfr2[1], #increment
    width=0.018,        # Default
    head_width=0.08,    # Default: 3 * width
    head_length=0.09    # Default: 1.5 * head_width
)
ax.add_patch(arrow3)
plt.text(0.20,-0.05, r'$2. F_{fr}$', fontsize=14,verticalalignment='top', horizontalalignment='left')

arrow3p1 = 2.5*cm 
arrow4 = pat.FancyArrow(
    arrow3p1[0], arrow3p1[1], vecfr2[0]/5,vecfr2[1]/5,#increment
    width=0.02,        # Default
...
ax.add_patch(arrow4)
plt.text(arrow3p1[0],arrow3p1[1], r'$a$', fontsize=14,verticalalignment='bottom', horizontalalignment='left')


plt.axis('equal')
plt.axis('off')
fig3.savefig('friction_yoyo_slant_right.pgf')#, facecolor=fig.get_facecolor(), edgecolor='none')
plt.show()
