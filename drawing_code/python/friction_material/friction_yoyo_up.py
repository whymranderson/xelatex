...
arrow2 = pat.FancyArrow(
    0.6*R+cm[0], cm[1],0,pb[0]*2/3,#increment
    width=0.018,        # Default
    head_width=0.08,    # Default: 3 * width
    head_length=0.09,    # Default: 1.5 * head_width
    color = 'r'
...
ax.add_patch(arrow2)
plt.text(0.6*R+cm[0],cm[1]+pb[0]*2/3, r'$F_{p}$', fontsize=14,verticalalignment='bottom', horizontalalignment='left')
...
plt.axis('equal')
plt.axis('off')
fig3.savefig('friction_yoyo_up.pgf')#, facecolor=fig.get_facecolor(), edgecolor='none')
plt.show()
