...
plt.text(arc1[0,0]+0.1,arc1[0,1], r'$\theta$', fontsize=20,verticalalignment='bottom', horizontalalignment='left')
...
plt.text(cm[0],cm[1], r'$CM$', fontsize=20,verticalalignment='bottom', horizontalalignment='right')
...
#ax.add_patch(arrow1)
#plt.text((tangentp[0]+vecfr[0]),(tangentp[1]+vecfr[1]), r'$F_{fr}$', fontsize=20,verticalalignment='top', horizontalalignment='left')

arrow2 = pat.FancyArrow(
    cm[0], cm[1], 0,-cm[1],#increment
    width=0.018,        # Default
    head_width=0.08,    # Default: 3 * width
    head_length=0.09    # Default: 1.5 * head_width
)
ax.add_patch(arrow2)
plt.text(cm[0],0, r'$mg$', fontsize=20,verticalalignment='top', horizontalalignment='left')

arrow3p1 = cm + 0.6*po
arrow3 = pat.FancyArrow(
    arrow3p1[0], arrow3p1[1], -0.5*vecfr[0],-0.5*vecfr[1],#increment
    width=0.05,        # Default
...
ax.add_patch(arrow3)
plt.text(arrow3p1[0],arrow3p1[1], r'$v$', fontsize=20,verticalalignment='bottom', horizontalalignment='left')



plt.axis('equal')
plt.axis('off')
#fig3.savefig('case1a.pgf')#, facecolor=fig.get_facecolor(), edgecolor='none')
plt.show()
