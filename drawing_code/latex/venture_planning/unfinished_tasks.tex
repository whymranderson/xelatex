...
\node [ item=2](texshop) at (1,1) {%
	...
	\nodepart[text width = 8.2cm]{two}
		...
		\item tetrahedron內切外接圖形也要入documentation examples
...
\node [ item=2, right = of texshop.north east, anchor = north west](webshop) {%
	...
	\nodepart[text width = 8.2cm]{two}
		\begin{itemize}
		\item add events, see 1/21/18$\bullet\bullet\circ\circ\circ\circ$
		...
		\item GO流程圖只保留特色框，其他框移到這邊後刪除，建立輸入流程圖。
		\item 修沒出現賽程
		...
		\end{itemize}
		\end{itemize}
		\item template folder set auto folder tracking with git 1/20/18
