...
\node [ item=2](a) at (1,1) {%
		...
		\item 應用subdoc讓case\_g20.tex及其他可以獨立運行，就不用去擔心編譯時期相對路徑找不到檔問題。
            	...
            	\item sciencefair typeset conti.$\bullet\bullet\bullet\circ$
		\item 將疊格繪圖方程modulize.$\bullet\bullet\circ\circ$
           \end{enumerate}
            };
...
\node [ item=2, below right = of incentive.south, anchor = north west](unique) {%
            ...
            \nodepart[text width = 8.2cm]{two}
            \begin{enumerate}
            	\item 使用LYX與SW來產生最高品質pdf文件。
		\item 高度自動化(省下很多細微瑣碎步驟)。
            	\item 結合不同程式與軟體來做多方應用，如網站建立，網頁編輯，資料庫建立，圖形繪製等。
           \end{enumerate}
            };
...
\node [ item=2,below = of a](small) {%
		...
		\item LYX export包含有被伺服圖檔的html檔，目前已有一寫下的流程，但還是要做成自動化較方便。
            	\item vec A不對稱問題？
            	\item 順著陀螺外型的powered by LYX SW字串，初步雛型已出來，需要finish off。$\bullet\bullet\bullet\circ$
	    \end{enumerate}
            };
...
\node [ item=2,below = of small](direct) {%
            ...
            \nodepart[text width = 8.2cm]{two}
            \begin{enumerate}
		\item openGL coordinate圖加入annotate docs.
