...
\node [ item=4](a) at (1,1) {%
		...
		\item 兵乓球碰撞後旋轉模擬。$\bullet\bullet\bullet\circ$
		\item 易德問$\omega_b$証。
		\item (排)加openGL coordinate圖。
		\item GUI照note中Fig 7下方的手寫紀錄來分類。
            	\item 結合疊格來做立體作圖？
            	\item 增加藍芽傳輸(沒有wifi時)
            	\item 與ejss準確度比較
...
\node [above right = of a.north east, align=center, anchor = south] (title){\includegraphics[width=0.4\textwidth]{../../figs/test_new_logo_chinese.png}\\ \LARGE 作業流程};
...
\node [above right = of thinking, align=center, anchor = south] (title){\parbox[c][][c]{0.3\textwidth}{\includegraphics[width=0.3\textwidth]{../../figs/test_new_logo_chinese.png}}\parbox[c][][c]{7cm}{\Huge 卡住kazoo}};

\node [ item=2, below right = of title.south, anchor = north west](gitlog) {%
