...
# v1.141 (c) 2012-2014 Silas S. Brown
...
%% === BEGIN JIANPU STAFF ===
    ...
    %% OLD version: \remove Staff_symbol_engraver \consists "Accidental_engraver" %% worked pre-2.18, but 2.18 results in missing barlines.
    %% Current version:
    \override StaffSymbol.color = #white %% works in 2.18 (and also in 2.15.40-1 etc), so keeping this one
    %% \override StaffSymbol #'line-count = #0 %% works in 2.18 but gives rather small barlines
    }
    { \new Voice="%s" {
...
def lyrics_start(voiceName="jianpu"):
    global lyricsPtr ; lyricsPtr += "I" # TODO: encapsulate
    return r'\new Lyrics = "%s" { \lyricsto "%s" { ' % (lyricsPtr,voiceName)
...
""" % (self.defines_done[figure],iType,figure2)
    ...
    if self.barPos==0 and self.onePage and self.barNo > 1 and not midi: ret += r"\noPageBreak "
    ...
    if not midi: ret += r"\applyOutput #'Voice #"+self.defines_done[figure]+"\n"
    ...
    if self.barPos%self.beatLength == 0 and (nBeams or self.barPos==self.barLength) and self.inBeamGroup:
...
for score in re.split(r"\sNextScore\s"," "+sys.stdin.read()+" "):
  ...
  for midi in [0,1]:
   ...
   for line in score.split("\n"):
    ...
    if line.startswith("LP:"):
        ...
        lyrics += lyrics_start()
        toAdd = ""
        if line and '1' <= line[0] <= '9' and line[1]=='.':
            # a verse number
            toAdd = r'\set stanza = #"%s" ' % line[:2]
            line = line[2:].strip()
   ...
   if midi: print midi_staff_start()
   else: print jianpu_staff_start(maxBeams=maxBeams) # TODO: multiple staffs?
   print '\n'.join(out)
   if midi: print midi_staff_end()
   else:
